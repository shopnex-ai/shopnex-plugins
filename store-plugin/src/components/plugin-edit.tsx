...
import { Gutter, LoadingOverlay, RenderTitle, TextInput, toast } from '@payloadcms/ui'
      ...
      <div className={`${baseClass}__header`}>
        ...
        <div className={`${baseClass}__block`}>
          ...
          <div className={`${baseClass}__info`}>
            <div className={`${baseClass}__author`}>By {plugin?.authorName || 'Unknown'}</div> |
            <div className={`${baseClass}__price`}>
              {plugin?.variants[0]?.price === 0 ? 'Free' : `From $${plugin?.variants[0]?.price}`}
            </div>
          </div>
        </div>
      </div>
