...
export const syncExistingWithStripe: CollectionBeforeChangeHookWithArgs =
    async (args) => {
        const { collection, data, operation, originalDoc, pluginConfig, req } =
            args;
                    ...
                    if (!data.stripeID) {
                        ...
                        if (logs) {
                            payload.logger.error(
                                `- There is no Stripe ID for this document, skipping.`
                            );
                        }
                    } else {
                        if (logs) {
                            payload.logger.info(
                                `- Syncing to Stripe resource with ID: '${data.stripeID}'...`
                            );
                        }
                        ...
                        try {
                            ...
                            const stripeResource = await stripe?.[
                                syncConfig?.stripeResourceType
                            ]?.update(data.stripeID, syncedFields);
                            ...
                            throw new APIError(
                                `Failed to sync document with ID: '${data.id}' to Stripe: ${msg}`
                            );
                        }
                    }
                }
